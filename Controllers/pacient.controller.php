<?php
require_once('../Models/cls_pacient.model.php');
$usuarios = new Clase_Pacient;
switch ($_GET["op"]) {
    case 'todos':
    ...
    case 'insertar':
        ...
        $datos = array(); //defino un arreglo
        $datos = $usuarios->insertar($cedula, $nombre, $fecha_nacimiento, $genero, $tipo_enfermedad, $correo, $test); //llamo al modelo de usuarios e invoco al procedimiento insertar
        echo json_encode($datos); //devuelvo el arreglo en formato json
        break;
    case 'actualizar':
        $paciente_id = $_POST["UsuarioId"];
        $cedula = $_POST["cedula"];
        $nombre = $_POST["nombre"];
        $fecha_nacimiento = $_POST["fecha_nacimiento"];
        $genero = $_POST["genero"];
        ...
        $datos = array(); //defino un arreglo
        $datos = $usuarios->actualizar($paciente_id, $cedula, $nombre, $fecha_nacimiento, $genero, $tipo_enfermedad, $correo, $test); //llamo al modelo de usuarios e invoco al procedimiento actual
        echo json_encode($datos); //devuelvo el arreglo en formato json
        break;
    case 'eliminar':
        $UsuarioId = $_POST["UsuarioId"]; //defino una variable para almacenar el id del usuario, la variable se obtiene mediante POST
    ...
    case 'login':
        ...
        if (empty($correo) || empty($contrasenia)) {
            header("Location:../index.php?op=1"); //llenar datos vacios
            exit();
        }
        try {
            $datos = array(); //defino un arreglo
            ...
            if (is_array($respuesta) and count($respuesta) > 0) {  // comparar si la variable "respuesta" tiene datos y es un arreglo
                ...
                if ($contrasenia == $respuesta["Contrasenia"]) {  //comparar la contraseña de la base con la contraseña que ingreso el usuario
                    ...
                    header("Location:../index.php?op=2"); //el usuario o la contraseña son incorrectos
                    exit();
                }
            } else {
                header("Location:../index.php?op=2"); //el usuario o la contraseña son incorrectos
            ...
            header("Location:../index.php?op=3"); // no se que error escribir  es para capturar un error de codigo
        }
        break;
