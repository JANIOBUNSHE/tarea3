...
function init() {
  $("#form_pacientes").on("submit", function (e) {
    guardaryeditar(e);
  });
}
...
var todos_controlador = () => {
  var todos = new Pacient_Model("", "", "", "", "", "", "", "", "todos");
  todos.todos();
}

var guardaryeditar = (e) => {
    e.preventDefault();
    var formData = new FormData($("#form_pacientes")[0]);
   
    var UsuarioId = document.getElementById("UsuarioId").value;
    if(UsuarioId > 0){
      var usuarios = new Pacient_Model('','','','','','',formData,'editar');
      usuarios.editar();
    }else{
      var usuarios = new Pacient_Model('','','','','','',formData,'insertar');
      usuarios.insertar();  
    }
};

var eliminar=(UsuarioId)=>{
    var eliminar = new Pacient_Model(UsuarioId, "", "", "", "", "", "", "eliminar");
    eliminar.eliminar();
}

var editar = (paciente_id) => {
    var uno = new Pacient_Model(paciente_id, "", "", "", "", "", "", "", "uno");
    uno.uno();
};
...
var cedula_repetida = () => {
    var cedula = $('#cedula').val();
    var usuarios = new Pacient_Model('',cedula,'','','','','','','cedula_repetida');
    usuarios.cedula_repetida();
}
var verifica_correo = () => {
    var Correo = $('#correo').val();
    var usuarios = new Pacient_Model('','','','','',Correo,'','','verifica_correo');
